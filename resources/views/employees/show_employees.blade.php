...
@section('page-header')
				...
				<div class="breadcrumb-header justify-content-between">
                    ...
                    <div class="d-flex my-xl-auto right-content">
                        <div>
                            <label class="pr-1 mb-3 mb-xl-0 mx-2">موظفين مفعلين</label>
                            <h5 class="text-center text-success">{{ $employeesActiveCount }}</h5>
                        </div>
                        <div>
                            <label class="pr-1 mb-3 mb-xl-0">موظفين غير مفعلين</label>
                            <h5 class="text-center text-danger">{{ $employeesDisableCount }}</h5>
                        </div>
                    </div>
				</div>
				<!-- breadcrumb -->
						...
						<div class="card mg-b-20">
							<div class="card-header pb-0">
								<div class="d-flex justify-content-right">
                                    ...
                                    @endcan
                                    @can('تصديرأكسيل')
                                        <a class="mx-1 modal-effect btn btn-outline-primary" href="{{ url('/' . $page ='employees/export') }}"><i class="fas fa-file-download"></i>&nbsp;تصدير اكسيل</a>
                                    @endcan
                                    @can('موظفين محذوفين')
                                        <a class="mx-1 modal-effect btn btn-outline-primary" href="{{ url('/' . $page='employees/trashEmployees') }}">موظفين محذوفين</a>
                                    @endcan
								</div>
							</div>
							<div class="card-body">
                                                    ...
                                                    <td>
                                                        <div class="dropdown">
                                                            ...
                                                            <div class="dropdown-menu tx-13">
                                                                @can('تعديل موظف')
                                                                <a class="dropdown-item" title="تعديل"  href="{{ route('employees.edit', $employee->id) }}"><i class="text-secondary far fa-edit"></i>&nbsp;&nbsp;تعديل</a>
                                                                @endcan
                                                                @can('حذف موظف')
                                                                <a class="dropdown-item" data-effect="effect-scale" data-employee_id="{{ $employee->id }}" data-employees_name="{{ $employee->employees_name }}" data-toggle="modal" title="حذف" href="#modaldemo8" ><i class="text-danger fas fa-trash-alt"></i>&nbsp;&nbsp;حذف</a>
                                                                @endcan
                                                                @can('عرض موظف')
                                                                <a class="dropdown-item"  title="عرض الملف" href="{{ route('employees.show', $employee->id) }}"><i class="text-success far fa-eye"></i>&nbsp;&nbsp;عرض</a>
                                                                @endcan
                                                                @if($employee->stauts == 'مفعل')
                                                                    <a class="dropdown-item text-danger"  title="تعطيل" href="{{ url('employees/disableEmployee/'.$employee->id) }}">تعطيل</a>
                                                                @elseif($employee->stauts == 'غير مفعل')
                                                                    <a class="dropdown-item text-success"  title="تفعيل" href="{{ url('employees/employeeActive/'.$employee->id)}}">تفعيل</a>
                                                                @endif
                                                            </div>
                                                        </div>
                                                    </td>

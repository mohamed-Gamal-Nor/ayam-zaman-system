...
@section('title')
تعديل فاتورة مشتريات - مصنع ايام زمان
@stop
@section('css')
...
@section('page-header')
				...
				<div class="breadcrumb-header justify-content-between">
					<div class="my-auto">
						<div class="d-flex">
							<h4 class="content-title mb-0 my-auto">مشتريات الموردين</h4><span class="text-muted mt-1 tx-13 mr-2 mb-0">/ تعديل فاتورة مشتريات</span>
						</div>
					</div>
				</div>
				<!-- breadcrumb -->
                    ...
                    <div class="col-lg-12 col-md-12">
						<div class="card">
							<div class="card-body">
								...
								<form action="{{ route("InvoicesReturns.update",$invoice->id) }}" id="selectForm" name="matarial" autocomplete="off" method="POST">
                                    @method('PUT')
                                    @csrf
                                    <div class="row row-sm mg-b-10">
                                        <div class="col-lg-3">
											<div class="form-group mg-b-0">
												...
												<input class="form-control" disabled id="code" type="text" value="{{ $invoice->supplier_id }}">
											</div>
										</div>
										<div class="col-lg-3">
											<div class="form-group mg-b-0">
												<label class="form-label">التاريخ:</label>
												<input class="form-control" disabled value="{{$invoice->invoice_Date}}" type="text">
											</div>
										</div>
										<div class="col-lg-3">
											...
											<select class="form-control select2" name="supplier_id" data-parsley-class-handler="#slWrapper" data-parsley-errors-container="#slErrorContainer" data-placeholder="اختار المورد" required="">
                                                ...
                                                @foreach ($suppliers as $supplier)
                                                    <option value="{{$supplier->id}}" @if ($supplier->id == old('supplier_id', $invoice->supplier_id ))
                                                        selected="selected"
                                                        @endif>
                                        ...
                                        <div class="col-lg-3">
											...
											<select class="form-control select2" name="store_id" data-placeholder="اختار المورد" required="">
                                                ...
                                                @foreach ($stores as $store)
                                                    <option value="{{$store->id}}" @if ($store->id == old('store_id ', $invoice->store_id  ))
                                                        selected="selected"
                                                        @endif>
                                                        {{$store->stores_name}}
                                                    </option>
                                    ...
                                    <div class="row row-sm">
										<div class="col-lg-3 col-sm-6">
											<div class="form-group mg-b-0">
												...
												<input class="form-control" disabled type="text" id="Beneficiary" value="{{$invoice->supplier->supplier_name}}">
											</div>
                                        </div>
                                        <div class="col-lg-3 col-sm-6">
											<div class="form-group mg-b-0">
												<label class="form-label">رقم الهاتف:</label>
												<input class="form-control" disabled type="text" id="phone" value="{{$invoice->supplier->supllier_phone }}">
											</div>
                                        </div>
                                        <div class="col-lg-3 col-sm-6">
											<div class="form-group mg-b-0">
												<label class="form-label">نوع التوريد:</label>
												<input class="form-control" disabled type="text" id="supply" value="{{$invoice->supplier->Type_of_supply }}">
											</div>
                                        </div>
                                        <div class="col-lg-3 col-sm-6">
											<div class="form-group mg-b-0">
												<label class="form-label">نوع الدفع:</label>
												<input class="form-control" disabled type="text" id="pay" value="{{$invoice->supplier->Type_of_pay }}">
											</div>
										</div>
                                    </div>
                                    <hr>
                                        ...
                                        <table name='matarial' class="table table-bordered mg-b-0 text-md-nowrap">
                                            ...
                                            <tbody>
                                                @foreach ($invoice->purchesReturns  as $key => $item)
                                                    <tr name="line_items">
                                                            ...
                                                            <div class="form-group mg-b-0">
                                                                <select class="form-control select2" name="material_id[]" required="">
                                                                    ...
                                                                    @foreach ($materials as $material)
                                                                        <option value="{{$material->id}}" @if ($material->id == old('material_id', $item->material_id   ))
                                                                            selected="selected"
                                                                            @endif>
                                                                            {{$material->materials_name}} -> {{$material->unit->unit_name}}
                                                                        </option>
                                                                    @endforeach
                                                        ...
                                                        <td>
                                                            <div class="form-group mg-b-0 ">
                                                                <input class="form-control price" name="price[]" value="{{ number_format($item->price,2) }}" type="text" placeholder="0.0 " required min="0">
                                                            </div>
                                                        </td>
                                                        <td>
                                                            <div class="form-group mg-b-0 ">
                                                                <input class="form-control Quantity" name="Quantity[]" value="{{ number_format($item->Quantity,2) }}" type="text" placeholder=" 0.0" required min="0">
                                                            </div>
                                                        </td>
                                                        <td>
                                                            <div class="form-group mg-b-0 ">
                                            ...
                                            <tfoot>
                                                ...
                                                <tr>
                                                    ...
                                                    <td colspan="3">
                                                        <textarea name="note" cols="100" spellcheck="true" rows="2">{{ $invoice->note }}</textarea>
                                                    </td>
                                                </tr>
                                            </tfoot>
                                        </table>
                                    ...
                                    <div class="col-12">
                                        ...
                                        <a class="btn btn-danger pd-x-20 mg-t-10" href="{{route("InvoicesReturns.index")}}">الغاء</a>
                                        <a class="btn btn-success pd-x-20 mg-t-10" href="{{route("InvoicesReturns.show",$invoice->id)}}">عرض الفاتورة</a>
                                    </div>
								</form>
                            </div>
